...
#  COCO
def prepare_coco_dataloaders(subset_num,dataloader_config,
                             dataset_root,
    ...
    if client > -1:
        ...
        dataloaders['train_subset_10000'] = _get_coco_loader(
            image_root, train_ann, train_ids, vocab,
            num_workers=num_workers, batch_size=batch_size,
            train=True,
            extra_annotation_path=val_ann,
        ...
        dataloaders['train_subset_eval_10000'] = _get_coco_loader(
            image_root, train_ann, train_ids, vocab,
...
def _get_coco_loader(image_root,
    ...
    if subset:
        if not os.path.exists('coco_subset_idx_file_q10000'):
            full_idx = [i for i in range(566435)]
            random.shuffle(full_idx)
            idx = full_idx[0: subset_num]
            idx.sort()
            if not os.path.exists('coco_subset_idx_file_q10000'):
                with open('coco_subset_idx_file_q10000', 'wb') as f:
                    pickle.dump(idx, f)

        if subset_num == 10000:
            with open('coco_subset_idx_file_q10000', 'rb') as f:
                idx = pickle.load(f)
