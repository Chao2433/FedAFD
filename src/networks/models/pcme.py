...
class PCME(nn.Module):
    ...
    def __init__(self, word2idx, config, mlp_local):
        ...
        if config.not_bert:
            ...
            self.txt_enc = BertModel.from_pretrained("./src/networks/models/bert-base-uncased")
            self.tokenizer = BertTokenizer.from_pretrained("./src/networks/models/bert-base-uncased")
    ...
    def image_forward(self, images):
        out = self.img_enc(images)['embedding'].float()
        return out

    def text_forward(self, sentences, lengths):
        return self.txt_enc(sentences, lengths)

    def berttext_forward(self, captions_word, lengths):
        inputs = self.tokenizer(captions_word, padding=True, return_tensors='pt')
        for a in inputs:
            inputs[a] = inputs[a].cuda("cuda:1")
        caption_output = self.txt_enc(**inputs)
        out = {'embedding': l2_normalize(self.linear(caption_output['last_hidden_state'][:, 0, :]))}  # [bsz, 768]
        out = out['embedding']
        return out
