...
class MultiHeadSelfAttention(nn.Module):
    ...
    def forward(self, x, mask=None):#[batch,256]
        # This expects input x to be of size (b x seqlen x d_feat)
        attn = self.w_2(self.tanh(self.w_1(x)))
        if mask is not None:
            mask = mask.repeat(self.n_head, 1, 1).permute(1, 2, 0).to(attn.device)
            # print('attn', attn.device, 'mask', mask.device)
...
class PIENet(nn.Module):
    ...
    def __init__(self, n_embeds, d_in, d_out, d_h, dropout=0.0):    #(1,2048,256,1024,0)
    ...
    def forward(self, out, x, pad_mask=None):   #[batch,256],[batch,49,2048]
