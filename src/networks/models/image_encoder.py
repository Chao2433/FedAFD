...
class EncoderImage(nn.Module):
    ...
    def forward(self, images):
        out_7x7 = self.cnn(images).view(-1, self.cnn_dim, 7, 7) #[batch,2048,7,7]
        pooled = self.avgpool(out_7x7).view(-1, self.cnn_dim)   #[batch,2048]
        out = self.fc(pooled)   #[batch,256]

        output = {}
        out_7x7 = out_7x7.view(-1, self.cnn_dim, 7 * 7) #[batch,2048,49]

        out, attn, residual = self.pie_net(out, out_7x7.transpose(1, 2))    #[batch,256]
