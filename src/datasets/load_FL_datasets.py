...
def get_FL_trainloader(dataset, data_root, num_clients, partition, alpha, batch_size):
    if dataset == 'cifar100':
        ...
        train_set = Language(name="AG_NEWS", train=True, transform=None, is_iid=False, root=data_root)
        test_set = Language(name="AG_NEWS", train=False, transform=None, is_iid=False, root=data_root)
    ...
    if dataset == "cifar100" or dataset == "cifar10":
        loader_map = {
            i: torch.utils.data.DataLoader(net_dataset_map[i], batch_size=batch_size, shuffle=True, num_workers=4) for
            i in net_dataset_map.keys()}
        test_loader = torch.utils.data.DataLoader(test_set, batch_size=batch_size , shuffle=False, num_workers=4)
    elif dataset == "AG_NEWS" or dataset == "YelpReviewPolarity":
        loader_map = {
            i: torch.utils.data.DataLoader(net_dataset_map[i], batch_size=batch_size, shuffle=True, num_workers=4,
        ...
        test_loader = torch.utils.data.DataLoader(test_set, batch_size=batch_size, shuffle=False, num_workers=4,
                                                  ...
                                                  collate_fn=caption_collate_fn)
    label_count_map = {}
    for client_id, dataset in net_dataset_map.items():
        labels = np.array(dataset.dataset.targets)[dataset.indices]
        unique_labels, label_counts = np.unique(labels, return_counts=True)
        label_count_map[client_id] = dict(zip(unique_labels, label_counts))

    print("Label Counts per Client:")
    for client_id, label_counts in label_count_map.items():
        print(f"Client {client_id}: {label_counts}")
