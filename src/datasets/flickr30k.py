...
class F30kCaptionsCap(Dataset):
    ...
    def iid(self, root='./data_partition/', num_users=15):
        """
        ...
        """
        pkl_path = root + 'client_flicker30k_iid.pkl'
    ...
    def non_iid(self, root='./data_partition/', num_users=15):
        pkl_path = root + 'client_flicker30k_noniid.pkl'
        if os.path.exists(pkl_path):
            print('noniid')
            dict_users = pickle.load(open(pkl_path, 'rb'))
        else:
            num_shards = 200
            num_imgs = int(len(self.data) / num_shards)
