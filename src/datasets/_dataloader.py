...
def _get_F30k_loader(vocab,
    ...
    coco_dataset = F30kCaptionsCap(train=True if split == 'train' else False,
                                   transform=_image_transform,
                                   target_transform=_caption_transform, client=client, is_iid=flag)
    if subset:
        if not os.path.exists('f30k_subset_idx_file_q10000'):
            # full_idx = [i for i in range(566435)]
            # random.shuffle(full_idx)
            # idx = full_idx[0: subset_num]
            # idx.sort()
            idx = [i for i in range(subset_num)]
            if not os.path.exists('f30k_subset_idx_file_q10000'):
                with open('f30k_subset_idx_file_q10000', 'wb') as f:
                    pickle.dump(idx, f)

        if subset_num == 10000:
            with open('f30k_subset_idx_file_q10000', 'rb') as f:
                idx = pickle.load(f)
    ...
    if client > -1:
        ...
        print(f'Loading F30k Caption: n_captions {len(coco_dataset)}...')
    return dataloader
...
def prepare_f30k_dataloaders(dataloader_config,
    ...
    if client > -1:
        ...
        dataloaders['train_subset_10000'] = _get_F30k_loader(
            vocab,
            num_workers=num_workers,
            batch_size=batch_size,
            train=True,
        ...
        dataloaders['train_subset_eval_10000'] = _get_F30k_loader(
            vocab,
            num_workers=num_workers,
