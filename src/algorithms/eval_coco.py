...
class COCOEvaluator(object):
    ...
    @torch.no_grad()
    def extract_features(self,key,writer,epoch,client_id, dataloader):
        ...
        # print('server', dataloader.batch_size)
        for images, captions, captions_word, caption_lens, ann_ids, image_ids, _ in self.pbar(dataloader):
            ...
            # if key == "train":
            #     writer.add_scalar(f'mm/Etrain_loss_{client_id}', loss, epoch)
            # if key == "test":
            #     writer.add_scalar(f'mm/test_loss_{client_id}', loss, epoch)
    ...
    @torch.no_grad()
    def evaluate(self,writer,epoch,idx, dataloader, n_crossfolds=None,
        ...
        extracted_features = self.extract_features(key,writer,epoch,idx,dataloader)
