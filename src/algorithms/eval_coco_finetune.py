...
class COCOEvaluator(object):
    ...
    @torch.no_grad()
    def extract_features(self,gff,writer,epoch,client_id,global_model,dataloader):
        ...
        global_model.to(self.extract_device)
        # if len(dataloader.dataset) == 50000:
        ...
        print('client',dataloader.batch_size)

        for images, captions, captions_word, caption_lens, ann_ids, image_ids, _ in self.pbar(dataloader):
            ...
            final_feature_image = gff(feature, fs)
            _image_features = final_feature_image
            output['image_features'] = final_feature_image

            feature = output['caption_features']
            fs = output_s['caption_features']

            final_feature_caption = gff(feature, fs)
            _caption_features = final_feature_caption
            output['caption_features'] = final_feature_caption
            ...
            for idx, image_id in enumerate(image_ids):  #5 caption -> 1 image
                ...
                cur_caption_idx += 1
        # cur_image_idx = int(image_ids_.size)
        if iid_to_cls:
    ...
    def evaluate_recall(self, q_features, g_features, q_labels, g_labels,
        ...
        if len(g_features) != len(g_labels):
            raise RuntimeError('length mismatch {}, {}'.format(g_features.shape,
                                                               g_labels.shape))
        n_queries = len(q_labels)   #5000
        n_galleries = len(g_labels) #25000
        best_pred_ranks = np.zeros(n_queries)

        # print('client i2t', batch_size)
        ...
        for q_indices in self.pbar(batch(range(n_queries), batch_size=batch_size)):
            ...
            for idx, q_idx in enumerate(q_indices):
                ...
                _pred_ranks = [torch.where(pred_ranks[idx] == pos_idx)[0][0].item() for pos_idx in pos_indices] #每个查询的正例排名
                best_pred_ranks[q_idx] = min(_pred_ranks) #每个查询的最佳匹配排名
    ...
    @torch.no_grad()
    def evaluate(self,gff, writer,epoch,idx,global_model,dataloader, n_crossfolds=None,
        ...
        extracted_features = self.extract_features(gff,writer,epoch,idx,global_model,dataloader)
