...
from src.algorithms.ClientTrainer_fedafd import ClientTrainer
from src.algorithms.MMClientTrainer_fedafd import MMClientTrainer
...
class MMFL(object):
    ...
    def create_model(self, args):
        ...
        if args.num_img_clients > 0:
            dataset = 'cifar100'
            self.img_trainloaders, test_set = get_FL_trainloader(dataset, args.data_root + "/cifar100",
        ...
        if args.num_txt_clients > 0:
            dataset = 'AG_NEWS'
            self.txt_trainloaders, test_set = get_FL_trainloader(dataset, args.data_root,
    ...
    def train(self, round_n):
        ...
        if not is_test:
            ...
            global_model = copy.deepcopy(self.engine.model)

        # global representations
        if self.args.agg_method == "SED" or self.args.BAA:
        ...
        for idx, trainer in enumerate(self.cur_trainers):
            ...
            trainer.run(self.global_img_feature, self.global_txt_feature, self.distill_index,
                        self._dataloaders['train_subset' + f'_{self.args.pub_data_num}'],global_model,trainer.client_idx,writer,self._dataloaders['test'])
    ...
    def distill(self, round_n, img_vec, txt_vec, img_num, txt_num, distill_index):
        ...
        def aggregation(i_vec, t_vec):
            print('ready to agg')
            if self.args.agg_method == "SED":
                contrastive_w = []
                for vec in i_vec:  # vec: [10000, 256], global_txt_feature: [10000, 256]
                    logits = torch.matmul(vec, self.global_img_feature.T)   # [10000, 10000]
                    exp_logits = torch.exp(logits)
                    log_prob = logits - torch.log(torch.sum(exp_logits, dim=1, keepdim=True))
                    contrastive_w.append(torch.diagonal(log_prob).reshape(1, -1)) # [1, 10000]
                ...
                contrastive_w = []
                for vec in t_vec:
                    logits = torch.matmul(vec, self.global_txt_feature.T)
                    exp_logits = torch.exp(logits)
                    log_prob = logits - torch.log(torch.sum(exp_logits, dim=1, keepdim=True))
        ...
        img_vecagg, txt_vecagg = aggregation(img_vec, txt_vec)

        self.img_vec = img_vecagg
        self.txt_vec = txt_vecagg
        ...
        for idx, (images, captions, captions_word, caption_lens, _, _, index) in tqdm(
            ...
            images = images.to(self.engine.device)
            captions = captions.to(self.engine.device)
            caption_lens = caption_lens.to(self.engine.device)
            ...
            if self.args.num_img_clients > 0 or self.args.num_mm_clients > 0:
                out_img = output['image_features']
            ...
            if self.args.num_txt_clients > 0 or self.args.num_mm_clients > 0:
                out_txt = output['caption_features']
                d_idx = operator.itemgetter(*index)(distill_dict)
